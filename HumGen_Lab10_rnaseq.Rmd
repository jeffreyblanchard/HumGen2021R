...
The below exercises are based on understanding and adapting code from a tutorial "RNA-Seq workflow: gene-level exploratory analysis and differential expression".  The exercises build on your knowledge of base R graphics and ggplot. Go slowly and try to understand each step. Add code chunks for exercises 4-9 at the end of your .Rmd file with the tutorial code.
...
In the `transformplot` section a data frame (dds) is created by binding together select columns from data objects `dds`, `vsd` and `rld` that have different transformations of the data. 

A. Use `head` to show the columns after this step.
...
C. Use `head` again to show what the titles in dds look like after the step `colnames(df)[1:2] <- c("x", "y")`
...
DESeq2 uses the Benjamini-Hochberg (BH) adjustment.  If we increase the bar for significance testing to 0.01. How many such genes are there? 
