...
A common error is for people to read in charactersm but forget the parentheses
...
Some more examples using the 23andMe data. To count the number of a factor in a category
...
However, count does not work within `summarize` See `tidyverse/dplyr` documentation for [Count observations by group](https://dplyr.tidyverse.org/reference/count.html). Insteas use `n()`. Hint: this is important for one of the exercises.
...
Use group_by() and summarize() to find the min positon, max position and total number of positions measured for each chromosome. 
